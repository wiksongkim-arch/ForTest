...
1. 打开 [GitHub Releases](https://github.com/wiksongkim-arch/ForTest/releases/latest)，下载 `ForTest-Windows-x64-Setup-0.2.15.exe`。
...
- **EIM 监听尚未发布**：执行计划要求先通过“当前登录用户自己的消息可完整回传”硬闸门；截至本版采用的官方 DWS v1.0.60，[事件监听说明](https://github.com/DingTalk-Real-AI/dingtalk-workspace-cli/blob/v1.0.60/skills/mono/references/products/event.md)明确自身消息会被 `isSelfLoop` 过滤，因此本版按 No-Go 规则未加入监听界面、后台引擎或轮询替代方案。
